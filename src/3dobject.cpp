...
void C3DObject::draw(void)
{
	...
	if (textures!=0) {
		if (display_list==-1) {
			...
			for(j=0, unique=0; j<ncaras; j++){
				for(m=0;m<unique;m++) if(t_used[m]==textures[j]) continue; // already inserted
				t_used[unique++]=textures[j];
				glBindTexture(GL_TEXTURE_2D,textures[j]);
				glBegin(GL_TRIANGLES);
				glColor3f(1,1,1);

				for(i=j;i<ncaras;i++) {
					if(textures[i]!=textures[j]) continue;
